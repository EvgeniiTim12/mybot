...
async def aaad(message: types.Message):
    ...
    if(SQL.check_admin(user)):
        ...
        if(not SQL.check_admin(admin)):
            ...
            await message.reply(str(trans("Додано",SQL.get_lang(user))+" @"+str(await get_userbyid(admin))))
    ...
    if(admin != user):
        if(SQL.check_admin(user)):
            if(SQL.check_admin(admin) and str(user) != str(admin)):
                ...
                await message.reply(str(trans("Видалено",SQL.get_lang(user))+" @"+str(await get_userbyid(admin))))
